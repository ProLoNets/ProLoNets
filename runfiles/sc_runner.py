        ...
        try:
            ...
            if find_new_step:
                ...
                tuple_out = run_episode(None,
                                        agent)  # This is a weird hack to make multithreading work. seems to slow things down so all threads can report in?
                if tuple_out[0] != -13:
                    ...
                    try:
                        while not queueue.empty():
                            try:
                                ...
                                print(e)
                                fake_out = [-13, None]
                            if fake_out[0] != -13:  # Assuming we didn't crash, save win/loss and the replay buffer
                                master_reward += fake_out[0]
                                running_reward_array.append(fake_out[0])
                                if fake_out[1]['rewards'][-1] > 0:
                                    k += 1
                                n += 1
                                if n < num_processes:
                                    agent.replay_buffer.extend(fake_out[1])
...
if __name__ == '__main__':
    ...
    AGENT_TYPE = args.agent_type  # 'prolo', 'random', 'fc', 'lstm',
    ...
    dim_in = 194
    dim_out = 44
    bot_name = AGENT_TYPE + 'SC_Macro'+'VeryEasy'
