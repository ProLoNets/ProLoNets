...
import numpy as np
from ProLoNets.agents.vectorized_prolonet import ProLoNet
import torch
...
def init_robot_nets(distribution='one_hot', use_gpu=False, vectorized=False, randomized=False, bot_name='usermadeprolo'):
    if randomized:
        dim_in = 28
        dim_out = 14
        init_weights = None
        ...
        act_fn = f'../study_models/finished_models/{bot_name}robot_actor_.pth.tar'
        val_fn = f'../study_models/finished_models/{bot_name}robot_critic_.pth.tar'
        action_network = load_prolonet(act_fn)
        value_network = load_prolonet(val_fn)
    return action_network, value_network


def init_marine_nets(distribution='one_hot', use_gpu=False, vectorized=False, randomized=False, bot_name='usermadeprolo'):
    if randomized:
        dim_in = 12
        dim_out = 9
        init_weights = None
        ...
        act_fn = f'../study_models/finished_models/{bot_name}Build_Marines_actor_.pth.tar'
        val_fn = f'../study_models/finished_models/{bot_name}Build_Marines_critic_.pth.tar'
        action_network = load_prolonet(act_fn)
        value_network = load_prolonet(val_fn)
    return action_network, value_network
