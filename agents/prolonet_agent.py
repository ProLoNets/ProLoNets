...
import sys
sys.path.insert(0, '../../')
from ProLoNets.opt_helpers import replay_buffer, ppo_update
from ProLoNets.agents.vectorized_prolonet_helpers import init_cart_nets, swap_in_node, add_level, init_marine_nets, \
    init_lander_nets, init_micro_net, init_adversarial_net, init_sc_nets, save_prolonet, load_prolonet, \
    init_robot_nets, init_fire_nets
import copy
import os
import random
import numpy as np
...
class DeepProLoNet:
    def __init__(self,
        ...
        if input_dim == 4 and output_dim == 2:  # CartPole
            ...
            if adversarial:
                self.action_network, self.value_network = init_adversarial_net(adv_type='micro',
                                                                               ...
                                                                               adv_prob=self.adv_prob)
                self.bot_name += '_adversarial' + str(self.adv_prob)
        elif input_dim == 28 and output_dim == 14:  # Fetch
            self.action_network, self.value_network = init_robot_nets(distribution,
                                                                      use_gpu,
                                                                      vectorized,
                                                                      randomized,
                                                                      bot_name.split('_')[0])
        elif input_dim == 12 and output_dim == 9:  # Build Marines
            self.action_network, self.value_network = init_marine_nets(distribution,
                                                                       use_gpu,
                                                                       vectorized,
                                                                       randomized,
                                                                       bot_name.split('_')[0])
        ...
        self.actor_opt = torch.optim.RMSprop(self.action_network.parameters(), lr=1e-4)
        self.value_opt = torch.optim.RMSprop(self.value_network.parameters(), lr=1e-4)
